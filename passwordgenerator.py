...
def finalpassword(length):
	...
	if int(length) > 4:
	    ...
	    combinedpassword = (atleast1ofeachlist) + list(remainingchars) #puts all values as a list and into variable "combinedpassword"
	    random.shuffle(combinedpassword) #shuffles the "combinedpassword" so the initial 4 will not have same characters
	    shuffledcombinedpassword = "".join(combinedpassword)
	    print "This is your password %s" %shuffledcombinedpassword
